...
import ReaderControls from './read/[chapter]/reader-controls';
...
export default async function BookReader({ work, chapterNumber }: { work: string; chapterNumber: number }) {
  ...
  if (!reading) {
    return (
      <main className="reader-unavailable">
        <p>This work or chapter is not currently available.</p>
      </main>
    );
  }
  ...
  const author = reading.bibliographic.creators.find((creator) => creator.role === 'author')?.label
    ...
    ?? 'Unknown creator';
  const underlyingRights = titleRecord?.rights.find((item) => item.component_type === 'underlying_work');
    ...
    <main className="reader-shell reader-shell--immersive">
      <ReaderControls
        ...
        edition={{
          ...
          releaseSequence: reading.release.release_sequence,
          masterSha256: reading.release.render_master_sha256,
          payloadSha256: titleRecord?.release.payload_sha256 ?? null,
          rights: underlyingRights
            ? `${underlyingRights.status.replaceAll('_', ' ')} · ${underlyingRights.use_scope}`
            : 'Rights information recorded in WNPH custody.',
        }}
      />

      <article className="chapter-reader">
        <header className="chapter-opening">
          <div className="chapter-edition-mark">Write Now recovered edition</div>
        ...
        <nav className="chapter-turn" aria-label="Chapter navigation">
          {reading.previous_chapter ? (
            <Link className="chapter-turn-card previous" href={chapterHref(reading.previous_chapter)}>
              <strong>← Previous chapter</strong>
            </Link>
          ) : (
            <Link className="chapter-turn-card previous" href="/library">
              <strong>← Library</strong>
            </Link>
          )}

          {reading.next_chapter ? (
            <Link className="chapter-turn-card next" href={chapterHref(reading.next_chapter)}>
              <strong>Continue reading →</strong>
            </Link>
          ) : (
            <Link className="chapter-turn-card next" href="/library">
