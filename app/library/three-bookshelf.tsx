...
function PhysicalBook({
  ...
  return (
    <group>
      ...
      {spineTexture ? (
        <mesh position={[-coverWidth / 2 - (hardcover ? .006 : .003), 0, 0]} rotation={[0, -Math.PI / 2, 0]} renderOrder={2}>
          <planeGeometry args={[thickness * .82, height * .94]} />
          <meshBasicMaterial map={spineTexture} toneMapped={false} side={THREE.DoubleSide} polygonOffset polygonOffsetFactor={-1} />
        </mesh>
      ) : null}
      {frontTexture ? (
        <mesh position={[0, 0, frontOuterZ + .008]} renderOrder={3}>
          <planeGeometry args={[coverWidth * .958, height * .958]} />
          <meshBasicMaterial map={frontTexture} toneMapped={false} side={THREE.DoubleSide} polygonOffset polygonOffsetFactor={-2} />
        </mesh>
      ) : null}
      {hardcover ? (
...
function BookMesh({ book, selected, hovered, onHover, onChoose }: {
  book: ShelfBook;
  selected: boolean;
  hovered: boolean;
  ...
  const shelfQuaternion = useMemo(() => new THREE.Quaternion().setFromEuler(new THREE.Euler(0, Math.PI / 2, lean)), [lean]);
  const hoverQuaternion = useMemo(() => new THREE.Quaternion().setFromEuler(new THREE.Euler(0, Math.PI / 2 - 7 * DEG, lean * .65)), [lean]);
  const frontQuaternion = useMemo(() => new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0)), []);
  ...
  useFrame((_, delta) => {
    ...
    if (Math.abs(selectionProgress.current - (selected ? 1 : 0)) < .0005) selectionProgress.current = selected ? 1 : 0;
    if (Math.abs(hoverProgress.current - (hovered && !selected ? 1 : 0)) < .0005) hoverProgress.current = hovered && !selected ? 1 : 0;
  ...
  return (
    <group ref={groupRef} position={[shelfX, shelfY, shelfZ]} quaternion={shelfQuaternion} onPointerOver={handlePointerOver} onPointerOut={handlePointerOut} onClick={(event) => { event.stopPropagation(); onChoose(volume); }}>
      <PhysicalBook volume={volume} height={height} coverWidth={coverWidth} thickness={thickness} coverTexture={coverTexture} spineTexture={spineTexture} fallbackCover={fallbackCover} />
    </group>
  );
}
...
function ShelfScene({ books, selectedId, hoveredId, onHover, onChoose }: {
  books: ShelfBook[];
  ...
  return (
    <>
      ...
      <directionalLight castShadow position={[3.1, 6.2, 7.4]} intensity={1.12} shadow-mapSize-width={2048} shadow-mapSize-height={2048} shadow-camera-left={-4.5} shadow-camera-right={4.5} shadow-camera-top={4.5} shadow-camera-bottom={-1.2} shadow-bias={-.00028} shadow-radius={5} />
      ...
      {books.map((book) => (
        <BookMesh key={book.volume.publicSlug} book={book} selected={selectedId === book.volume.publicSlug} hovered={hoveredId === book.volume.publicSlug && selectedId !== book.volume.publicSlug} onHover={onHover} onChoose={onChoose} />
      ))}
    </>
  );
}
...
function buildShelfBooks(volumes: LibraryVolume[]) {
  const dimensions = volumes.map((volume, index) => {
    ...
    return { volume, height, coverWidth: height * (2 / 3), thickness: Math.max(.22, volume.width / 100), shelfZ: ((index % 3) - 1) * .012 };
  });
  const gap = .016;
  const totalWidth = dimensions.reduce((sum, item) => sum + item.thickness, 0) + Math.max(0, dimensions.length - 1) * gap;
...
export default function ThreeBookshelf({ volumes }: ThreeBookshelfProps) {
  ...
  const selectedVolume = useMemo(() => volumes.find((volume) => volume.publicSlug === selectedId) ?? null, [selectedId, volumes]);
  ...
  return (
    <section className={styles.stage} aria-label="Interactive bookshelf">
      <Canvas className={styles.canvas} shadows dpr={[1, 1.75]} camera={{ position: [.16, 1.86, 11.55], fov: 23, near: .1, far: 50 }} gl={{ alpha: true, antialias: true, powerPreference: 'high-performance' }} onCreated={({ gl }) => {
        gl.shadowMap.enabled = true;
        gl.shadowMap.type = THREE.PCFSoftShadowMap;
        gl.outputColorSpace = THREE.SRGBColorSpace;
        gl.toneMapping = THREE.ACESFilmicToneMapping;
        gl.toneMappingExposure = 1.02;
      }} onPointerMissed={() => { setSelectedId(null); setHoveredId(null); document.body.style.cursor = ''; }}>
        <ShelfScene books={books} selectedId={selectedId} hoveredId={hoveredId} onHover={setHoveredId} onChoose={chooseVolume} />
      </Canvas>
      <div className={`${styles.detail} ${detailVisible && selectedVolume ? styles.detailVisible : ''}`}>
        {selectedVolume ? (
          <>
            ...
            <p className={styles.facts}>{selectedVolume.demo ? `${selectedVolume.binding} · temporary shelf example` : `${selectedVolume.chapterCount} chapters · ${selectedVolume.mediaCount} illustrations`}</p>
            <button className={styles.returnButton} type="button" onClick={() => { setSelectedId(null); setHoveredId(null); }}>Return to shelf</button>
          </>
        ) : null}
      </div>
    </section>
