...
function stepSpring(
  ...
  if (Math.abs(target - state.value) < .00035 && Math.abs(state.velocity) < .0025) {
    state.value = target;
    state.velocity = 0;
  }
}
...
function BookMesh({
  ...
  const selectionSpring = useRef<SpringState>({ value: selected ? 1 : 0, velocity: 0 });
  const hoverSpring = useRef<SpringState>({ value: 0, velocity: 0 });
  ...
  const hoverQuaternion = useMemo(
    () => new THREE.Quaternion().setFromEuler(new THREE.Euler(0, Math.PI / 2 - 6 * DEG, lean * .72)),
    [lean],
  );
  ...
  useFrame((_, delta) => {
    ...
    stepSpring(selectionSpring.current, selected ? 1 : 0, delta, 42, 13.2);
    stepSpring(hoverSpring.current, hovered && !selected ? 1 : 0, delta, 82, 18.2);

    const selectionT = THREE.MathUtils.clamp(selectionSpring.current.value, 0, 1);
    const hoverT = THREE.MathUtils.clamp(hoverSpring.current.value, 0, 1) * (1 - selectionT);

    const pullT = THREE.MathUtils.smoothstep(selectionT, 0, .34);
    const faceT = THREE.MathUtils.smoothstep(selectionT, .28, 1);
    const pullDistance = .22;
    const selectedZ = .92;
    const selectedX = -.88;

    group.position.set(
      THREE.MathUtils.lerp(shelfX, selectedX, faceT),
      shelfY + .004 * hoverT + .012 * pullT + .014 * faceT,
      shelfZ + .052 * hoverT + pullDistance * pullT + (selectedZ - pullDistance) * faceT,
    );
...
export default function ThreeBookshelf({ volumes }: ThreeBookshelfProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setDetailVisible(true), 760);
